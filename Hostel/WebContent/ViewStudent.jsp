...
$("#printbutton").show();
	printDivCSS = new String(
			'<link href="CSS/style.css" rel="stylesheet" type="text/css" />')
	printDivCSS1 = new String(
	'<link href="CSS/login.css" rel="stylesheet" type="text/css" />')

	function printDiv(divId) {
		$("#printbutton").hide();
		window.frames["print_frame"].document.body.innerHTML = printDivCSS1+printDivCSS
								...
								<table align="left" cellspacing="10px">
									<tr>
										<td><a href="MyGoal.jsp?id=<%=id%>"> <img
												src="/Hostel/ViewPhoto?id=<%=id%>&type=student" height="90"
												width="90" id="student" border="1" alt="No Photo Available" /></a>
										</td>

										<td><a href="MyGoal.jsp?id=<%=id%>"> <img
												src="/Hostel/ViewPhoto?id=<%=id%>&type=Mother" height="90"
												width="90" id="mother" border="1" alt="No Photo Available"
												style="background-color: gray;" /></a></td>

										<td><a href="MyGoal.jsp?id=<%=id%>"> <img
												src="/Hostel/ViewPhoto?id=<%=id%>&type=Father" height="90"
												width="90" id="father" border="1" alt="No Photo Available" /></a>
										</td>
									</tr>
								...
								<table width="100%">
									...
									<tr>
										...
										<td><label><b><%=rs.getInt("std")%></b></label>
											...
											<%
												String query2 = "select (nextyear) from studentyear where studentid='"
															+ id + "'";
 ...
 %> <%-- 		<% for(int i=rs.getInt("Fromyear");i<=rs.getInt("toYear");i++){%><%=i%>
									...
									<tr>
										...
										<td><label class="I18Nmsg" name="stucast"><%=rs.getString("stucast")%></label>
											<label class="I18Nmsg"><%=rb.getString("stu.subcast")%></label>
											<label class="I18Nmsg" name="stusubcast"><%=rs.getString("subcast")%></label>
										</td>
									...
									<tr>
										...
										<td><label class="I18Nmsg" name="sturegion"><%=rs.getString("region")%></label>
											...
											<label class="I18Nmsg" name="stunationality"><%=rs.getString("nationality")%></label>
										</td>
									...
									<tr>
										...
										<td><label name="stutele" size="25"><%=rs.getString("stutel")%></label>
											<label class="I18Nmsg"><%=rb.getString("stu.mo")%></label> <label
											name="stumo" size="25"><%=rs.getString("stumob")%></label></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td><label name="stureltel" size="25"><%=rs.getString("stutele")%></label>
											<label class="I18Nmsg"><%=rb.getString("stu.relativemo")%></label>
									...
									<tr>
										<td><label class="I18Nmsg"><%=rb.getString("stu.activity")%>
										</label></td>
									...
									<tr>
										<td></td>
										<td><input id="printbutton" type="button" class="login_button"
											value="print" onClick="printDiv('page')" /></td>
									</tr>
								</table>
								<%
									}
